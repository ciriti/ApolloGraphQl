package com.arrow.core.algebric
...
sealed class MpTry<out A> : TryOf<A> {
    ...
    companion object {
        ...
        fun <A> raise(e: Throwable): MpTry<A> =
            Failure(e)
    ...
    fun filter(p: Predicate<A>): MpTry<A> =
            flatMap { if (p(it)) Success(it) else Failure(
                TryException.PredicateException("Predicate does not hold for $it")
            )
            }
...
//            flatMap { a -> f(a).fold({ Failure(TryException.PredicateException("Predicate does not hold")) }, { MpTry.just(it) }) }
    ...
    fun failed(): MpTry<Throwable> =
            fold(
                    ...
                    {
                        Failure(
                            TryException.UnsupportedOperationException(
                                "Success"
                            )
                        )
                    }
 ...
 */
fun <B> TryOf<B>.recoverWith(f: (Throwable) -> TryOf<B>): MpTry<B> = fix().fold({ f(it).fix() }, {
    MpTry.Success(it)
})
 ...
 */
fun <B> TryOf<B>.recover(f: (Throwable) -> B): MpTry<B> = fix().fold({
    MpTry.Success(f(it))
}, { MpTry.Success(it) })

fun <A> (() -> A).try_(): MpTry<A> =
    MpTry(this)

fun <A> A.success(): MpTry<A> =
    MpTry.Success(this)

fun <A> Throwable.failure(): MpTry<A> =
    MpTry.Failure(this)
